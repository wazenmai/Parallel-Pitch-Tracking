...
int max_freq = 1000;
int min_freq = 40;
const int F = 8; // 16 frames per block
...
int main(int argc, char **argv) {
    ...
    // 1 block -> 64 thread -> 16 frames
    const int M = (num_frames + F - 1) / F;
    printf("Number of blocks: %d, Number of threads in block: 64\n", M);
