...
where_is_happyness.wav
            Type  Time(%)      Time     Calls       Avg       Min       Max  Name
 GPU activities:   99.39%  381.52ms         1  381.52ms  381.52ms  381.52ms  calculate_pitch
*/
...
int max_freq = 1000;
int min_freq = 40;
const int F = 8; // 8 frames per block
...
__global__ void calculate_pitch(int* data, int* pitch, int frame_size, int num_frames, int n1, int n2, int fs) {
    ...
    int calculate = frame_size / 4; // the range of each thread need to calculate

    int slice_id = tid % 4;
    int frame_id_in_block = tid / 8;
    int frame_id_global = bid * F + frame_id_in_block;
    int sample_start_in_frame = slice_id * calculate;
    int start = (bid + frame_id_in_block) * frame_size + sample_start_in_frame;
    // if (bid == 1) {
    ...
    double max_acf_own = 0.0;
    int max_acf_index_own = 0;
    int self_start = max(n1, sample_start_in_frame);
    int self_end = min(n2, sample_start_in_frame + calculate);
    for (int shift = self_start; shift < self_end; shift++) {
        double out = 0.0;
        double deminator = 0.0;
        for (int j = 0; j < frame_size - shift; j++) {
            out += data_s[frame_id_in_block][j] * data_s[frame_id_in_block][j + shift];
            deminator += data_s[frame_id_in_block][j] * data_s[frame_id_in_block][j] + data_s[frame_id_in_block][j + shift] * data_s[frame_id_in_block][j + shift];
        }
    ...
    data_s[frame_id_in_block][sample_start_in_frame] = max_acf_own;
    data_s[frame_id_in_block][sample_start_in_frame + 1] = max_acf_index_own;
    __syncthreads();

    // utilize shared memory space to exchange max acf
    if (slice_id == 0) { // represent the frame it calculate
        double max_acf = max_acf_own;
        int max_acf_index = max_acf_index_own;
        if (data_s[frame_id_in_block][sample_start_in_frame + calculate] > max_acf) {
            max_acf = data_s[frame_id_in_block][sample_start_in_frame + calculate];
            max_acf_index = data_s[frame_id_in_block][sample_start_in_frame + calculate + 1];
        }
        if (data_s[frame_id_in_block][sample_start_in_frame + calculate * 2] > max_acf) {
            max_acf = data_s[frame_id_in_block][sample_start_in_frame + calculate * 2];
            max_acf_index = data_s[frame_id_in_block][sample_start_in_frame + calculate * 2 + 1];
        }
        if (data_s[frame_id_in_block][sample_start_in_frame + calculate * 3] > max_acf) {
            max_acf = data_s[frame_id_in_block][sample_start_in_frame + calculate * 3];
            max_acf_index = data_s[frame_id_in_block][sample_start_in_frame + calculate * 3 + 1];
        }
...
int main(int argc, char **argv) {
    ...
    calculate_pitch<<<M, 32>>>(data_device, pitch_device, frame_size, num_frames, n1, n2, fs);
