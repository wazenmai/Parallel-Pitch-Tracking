// No share memory, no low precision, Correct.
/*
walking.wav
...
int max_freq = 1000;
int min_freq = 40;
const int F = 4; // # of frames per block
const int N = 256; // # of threads per frame
const int T = 1024; // # of threads per block
...
__global__ void calculate_pitch(int* data, int* pitch, int frame_size, int num_frames, int n1, int n2, int fs) {
    ...
    // 8 thread a frame
    const int quotient = frame_size / N; // the range of each thread need to calculate
    const int mod = frame_size % N;
    
    const int slice_id = tid % N;
    const int frame_id_in_block = tid / N;
    const int frame_id_global = bid * F + frame_id_in_block;

    const int calculate = (slice_id < mod) ? quotient + 1 : quotient;
    const int sample_start_in_frame = quotient * slice_id + ((slice_id < mod) ? slice_id : mod);
    const int sample_start_global = frame_id_global * frame_size + sample_start_in_frame;
    const int start = frame_id_global * frame_size;

    __shared__ int data_s[F][1536];
    // Load global memory data to shared memory data_s
    for (int i = 0; i < calculate; i++) {
        data_s[frame_id_in_block][sample_start_in_frame + i] = data[sample_start_global + i];
    }
    __syncthreads();

    __shared__ double acf[F][N];
    __shared__ int acf_index[F][N];

    double max_acf_own = 0.0;
    int max_acf_index_own = 0;
    const int shift_start = max(n1, sample_start_in_frame);
    const int shift_end = min(n2, sample_start_in_frame + calculate);
    
    for (int shift = shift_start; shift < shift_end; shift++) { // shift inside the frame
        double out = 0.0;
        double denominator = 0.0;
        for (int j = 0; j < frame_size - shift; j++) {
            out += data_s[frame_id_in_block][j] * data_s[frame_id_in_block][j + shift];
            denominator += data_s[frame_id_in_block][j] * data_s[frame_id_in_block][j] + data_s[frame_id_in_block][j + shift] * data_s[frame_id_in_block][j + shift];
        }
        double acf = (denominator > 0) ? 2 * out / denominator : 0;
        if (acf > max_acf_own) {
            max_acf_own = acf;  
            max_acf_index_own = shift;
        }
    }
    acf[frame_id_in_block][slice_id] = max_acf_own;
    acf_index[frame_id_in_block][slice_id] = max_acf_index_own;
    __syncthreads();
    ...
    if (slice_id == 0) { // represent the frame it calculate
        ...
        for (int i = 1; i < N; i++) {
            if (acf[frame_id_in_block][i] > max_acf) {
                max_acf = acf[frame_id_in_block][i];
                max_acf_index = acf_index[frame_id_in_block][i];
            }
        }
