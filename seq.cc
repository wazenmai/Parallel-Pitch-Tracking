...
int main(int argc, char **argv) {
    ...
    int f_num = 0;
    for (long f = 0; f < num_samples; f += frame_size) {
        // printf("Start from %ldth sample, ", f);
        f_num += 1;
        for (int shift = 0; shift < max_shift; shift++) {
            ...
            */
            ...
            for (long j = 0; j < frame_size - shift; j++) {
                out += data[0][f + j] * data[0][f + j + shift];
                deminator += data[0][f + j] * data[0][f + j] + data[0][f + j + shift] * data[0][f + j + shift];
            }
            if (deminator > 0) {
    ...
    // normalizeAndScale(data[0], num_samples, 10);

    // Yin yin;
    // float* pitch = (float*)malloc(sizeof(float) * num_frames);
    // float yin_threshold = 0.5;
    // for (int f = 0; f < num_frames; f++) {
    //     pitch[f] = 0.0;
    //     if (volume[f] < volume_threasold) {
    //         continue;
    //     }
    //     Yin_init(&yin, frame_size, yin_threshold);
    //     pitch[f] = Yin_getPitch(&yin, data[0] + f * frame_size);
    //     printf("frame %d: pitch=%f with probability %f\n", f, pitch[f], Yin_getProbability(&yin));
    // }
