...
public:
    ...
    MIDItrack& operator[] (unsigned trackno)
    {
        ...
        if(result.empty())
        {
            ...
            // Meta 0x51 (tempo):
            result.AddMetaEvent(0x51,3,  tempo>>16, tempo>>8, tempo); // 15, 3906
        }
        return result;
    }
...
int main()
{
    ...
    //             printf("KeyOff at row %d with note %d\n", row, keys_on[c]);
    //             keys_on[c] = -1;
    //             if(note == x) continue;
    //             file[0].KeyOn(c, keys_on[c] = note+add, vol);
    //             printf("KeyOn at row %d with note %d\n", row, keys_on[c]);
    //         }
    //         file[0].AddDelay(160);
    //     }
    ...
    int num_of_frames = 53;
    file[0].Patch(0, 0);
    double pitch[53] = {0, 0, 50, 0, 0, 50, 50, 50, 0, 0, 0, 0, 0, 0, 0, 0, 0, 50, 0, 0, 0, 0, 0, 50, 50, 50, 50, 50, 50, 50, 0, 0, 0, 0, 50, 50, 49, 49, 49, 49, 0, 0, 0, 0, 49, 48, 49, 49, 49, 50, 0, 0, 0};
    int key_on = -1;
    for (int i = 0; i < num_of_frames; ++i) {
        if (pitch[i] <= 35 || pitch[i] >= 85) {
            file[0].AddDelay(100);
            if (key_on != -1) {
                file[0].KeyOff(0, key_on, 0x20);
                printf("KeyOff at row %d with note %d\n", i, key_on);
                key_on = -1;
            }
        } else {
            ...
            printf("KeyOff at row %d with note %d\n", i, key_on);
            key_on = -1;
            file[0].KeyOn(0, key_on = round(pitch[i]), 0x4B);
            printf("KeyOn at row %d with note %d\n", i, key_on);
            file[0].AddDelay(100);
        }
    }
    ...
    FILE* fp = std::fopen("test1.mid", "wb");
